...
Game.load = () => {
    ...
    Game.upgrade = class {
        constructor(name, requirement, price, desc, img, effects) {
            ...
            this.obj = () => {
                var newObj = c({ tag: 'div', classList: 'upgrade', id: `upgrade-${removeSpecials(name)}`, children: [c({ tag: 'img', src: img })] });
    ...
    Game.updateBots = () => {
        ...
        Game.bots.forEach((bot, index) => {
            ...
            document.querySelector(`#bot-${removeSpecials(bot.name)}`).addEventListener('mouseleave', () => {
                document.querySelector('#hover').style.display = 'none';
            })
        })
    }
    ...
    Game.tentick = () => {
        ...
        if (JSON.stringify(Game.prevUpgrades) !== JSON.stringify(Game.availUpgrades)) {
            ...
            Game.availUpgrades.forEach(upgrade => {
                ...
                f(`#upgrade-${removeSpecials(upgrade)}`).on('click', () => {
        ...
        Game.availUpgrades.forEach(upgrade => {
            var up2 = Game.totalUpgrades.filter(a => a.name === upgrade)[0]
            if (Game.shapes >= up2.price) {
                f(`#upgrade-${removeSpecials(up2.name)}`).classList.remove('disabled')
            } else {
                f(`#upgrade-${removeSpecials(up2.name)}`).classList.add('disabled')
            }
        })
